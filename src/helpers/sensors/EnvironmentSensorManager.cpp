...
bool EnvironmentSensorManager::querySensors(uint8_t requester_permissions, CayenneLPP& telemetry) {
  ...
  if (requester_permissions & TELEM_PERM_ENVIRONMENT) {
    ...
    if (motion.valid) {
      const auto& v = motion.values;
      telemetry.addAccelerometer(next_available_channel, v.ax, v.ay, v.az);
      // Cayenne's signed 0.01 dps representation cannot hold the full +/-500 dps.
      // Clamp only the encoded telemetry; rules and text use the full reading.
      telemetry.addGyrometer(next_available_channel,
        fmaxf(-327.68f, fminf(327.67f, v.gx)),
        fmaxf(-327.68f, fminf(327.67f, v.gy)),
        fmaxf(-327.68f, fminf(327.67f, v.gz)));
      telemetry.addTemperature(next_available_channel++, v.temperature);
    }
    #endif
